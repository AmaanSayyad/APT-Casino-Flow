...
export default function Navbar() {
    ...
    const initializeFlowWallet = async () => {
      try {
        ...
        if (FlowWalletService.isConnected()) {
          ...
          // Load balance
          try {
            const balance = await FlowWalletService.getFlowBalance();
            setFlowBalance(balance);
          } catch (error) {
            console.warn('Could not load Flow balance:', error);
          }
        }
      } catch (error) {
  ...
  }, [isConnected, address, flowBalance, isWalletReady]);
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const balanceInFlow = parseFloat(userBalance || '0');
      if (balanceInFlow <= 0) {
      ...
      console.log('🔍 Flow account object:', address);
      console.log('🔍 Flow account address:', address);
      console.log('🔍 Flow account address type:', typeof address);
      ...
      const response = await fetch('/api/withdraw', {
        ...
        body: JSON.stringify({
          userAddress: address,
          amount: balanceInFlow
        })
      });

      const result = await response.json();
      console.log('🔍 Flow withdraw API response:', result);
  ...
  const handleDeposit = async () => {
    ...
    // Check deposit limits (using Flow limits)
    if (amount < 0.001) {
      notification.error('Minimum deposit amount is 0.001 FLOW');
      return;
    }
    
    if (amount > 100) {
      notification.error('Maximum deposit amount is 100 FLOW');
      return;
    }
    ...
    try {
      ...
      // Use Flow wallet service for deposit
      const txHash = await FlowWalletService.transferFlow(
        FLOW_CONFIG.TREASURY.ADDRESS,
        amount
      );
      
      console.log('✅ Flow deposit transaction sent:', txHash);
      
      // Wait for transaction confirmation
      notification.info(`Flow transaction sent! Hash: ${txHash.slice(0, 10)}...`);
      ...
      notification.success(`Successfully deposited ${amount} FLOW to casino treasury! TX: ${txHash.slice(0, 10)}...`);
        ...
        {isClient && showBalanceModal && createPortal(
          ...
          >
            ...
            >
              ...
              {/* Withdraw Section */}
              <div className="mb-4">
                <h4 className="text-sm font-medium text-white mb-2">Withdraw All FLOW</h4>
                <button
                ...
                >
                  {isWithdrawing ? (
                    ...
                    parseFloat(userBalance || '0') > 0 ? 'Withdraw All FLOW' : 'No Balance'
                  ) : 'Connect Flow Wallet'}
